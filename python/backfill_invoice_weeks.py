...
Business rules:
- invoice_monday is the NEXT Monday after delivery_date (strictly after; if delivery is Monday, use the following Monday).
- If pickup_date is on/after invoice_monday, push invoice_monday forward by 7 days.
- Store invoice_monday as a UTC datetime at midnight.
- Store invoice_week_id as a YYYY-MM-DD string (same date as invoice_monday).
...
def next_monday_after(delivery_dt_utc: datetime) -> datetime:
    """
    Return the next Monday after the delivery date (strictly after),
    as a UTC midnight datetime.
    """
    delivery_day = delivery_dt_utc.date()
    weekday = delivery_day.weekday()  # Monday=0 ... Sunday=6

    days_until_next_monday = (7 - weekday) % 7
    if days_until_next_monday == 0:
        days_until_next_monday = 7

    invoice_day = delivery_day + timedelta(days=days_until_next_monday)
    return datetime.combine(invoice_day, time(0, 0), tzinfo=UTC)


def compute_invoice_fields(
    pickup_dt_utc: datetime, delivery_dt_utc: datetime
) -> Tuple[datetime, str]:
    """
    Apply business rules to compute invoice_monday and invoice_week_id.
    """
    invoice_monday = next_monday_after(delivery_dt_utc)

    # If pickup is on/after invoice Monday, the load belongs to the next invoice week.
    if pickup_dt_utc >= invoice_monday:
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Backfill invoice_monday and invoice_week_id for all loads."
    )
    p.add_argument(
        "--mongo-uri",
...
def main() -> int:
    ...
    def flush_updates() -> None:
        ...
        # modified_count can be 0 for some ops if the values were already identical
        # (rare here because we only enqueue changes), but we'll treat matched==updated intent.
    ...
    try:
        for doc in cursor:
            ...
            # If not dry-run, we'll count updates based on bulk_write modified_count.
